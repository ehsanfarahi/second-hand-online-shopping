...
return (
    ...
    <GoBack />
       ...
       <DetailLeftSide/>
        <DetailRightSide product={product} setChatBox={setChatBox} urlSocialMediaPlatforms={urlSocialMediaPlatforms} setDisplayShareSocial={setDisplayShareSocial} favorite={favorite} setFavorite={setFavorite} setBottomAdDisplay={setBottomAdDisplay} />
       </div>
       <div className="mb-14">
    ...
    {loading && <Spinner/>}
    <BottomAddToFavorite product={product} favorite={favorite} setFavorite={setFavorite} updateFavorite={updateFavorite} setUpdateFavorite={setUpdateFavorite} bottomAdDisplay={bottomAdDisplay} setBottomAdDisplay={setBottomAdDisplay} />
    </>
)
}
...
function BottomAddToFavorite({product, favorite, updateFavorite, setFavorite, setUpdateFavorite, bottomAdDisplay, setBottomAdDisplay}) {
    ...
    return (
        <div className="bottomAddToFav-container">
           ...
           <div className="flex items-center">
            ...
            <div className="flex items-center">
                <p className="pr-6 text-2xl">${product.discount ? (product.price - (product.discountPercentage / 100 * product.price)) : Number(product.price)?.toFixed(2)}</p>
                {!favorite && getFavId?.includes(product.id) ? <button onClick={handleRemoveFromFavorite} className="bottomAddToFav-btn">Remove from favorite ({updateFavorite})</button> : <button onClick={handleAddToFavorite} className="bottomAddToFav-btn">Add to favorite ({updateFavorite || 0})</button>}
            </div>
           </div>
           <div className={`border-2 w-fit border-slate-700 cursor-pointer rounded-[50%] p-1 absolute left-[50%] translate-x-[-50%] ${bottomAdDisplay ? "top-[1.2rem]" : "top-[-3.55rem]"} `}>
            {bottomAdDisplay && getFavId?.includes(product.id) ? <FaHeartCircleMinus onClick={()=>setBottomAdDisplay(e=>!e)} className="text-3xl" /> : bottomAdDisplay && !favorite ? <TbHeartMinus onClick={()=>setBottomAdDisplay(e=>!e)} className="text-3xl" /> : getFavId?.includes(product.id) ? <FaHeartCirclePlus onClick={()=>setBottomAdDisplay(e=>!e)} className="text-3xl" /> : <TbHeartPlus onClick={()=>setBottomAdDisplay(e=>!e)} className="text-3xl" />}
            </div>
        </div>
    )
}
...
function DetailRightSide({product, setChatBox, setDisplayShareSocial, favorite, setFavorite, setBottomAdDisplay}) {
    ...
    <div className="ml-10 sm:ml-0">
        ...
        <div className="flex justify-between items-center">
            ...
            <span  className="hidden md:hidden sm:block mr-2 text-3xl cursor-pointer bg-orange-100 p-3 rounded-[50%]"><GrShareOption onClick={()=>setExpandMobileShare(true)} /></span>
            <span  className=" sm:pr-4 sm:hidden text-4xl md:text-3xl sm:text-3xl cursor-pointer bg-orange-100 p-2 rounded-[50%]">{expandShare ? <RxCross2 onClick={()=>setExpandShare(e=>!e)} /> : <GrShareOption onClick={()=>setExpandShare(e=>!e)} />}</span>
       ...
       <div className="flex justify-between items-center">
       <h2 className="mt-2 text-2xl sm:text-3xl md:text-xl font-semibold text-orange-500">{product.title}</h2>
       <span>{product.date}</span>
       </div>
        <p className="text-slate-400 mt-1 sm:text-xl">{product.model}</p>
        <div className="flex items-center mt-4">
        <SlLocationPin className="mr-2" />
        <p className="text-slate-600 text-sm sm:text-xl">{product.sellerLocation}</p>
        </div>
        <div className="mt-2 flex items-center"><LuEye /> <p className="pl-2">20 persons have seen this ad</p></div>

        <div className="mt-4 flex items-center sm:mt-8">
        <p className="font-bold text-2xl sm:text-2xl text-slate-700">${product.discount ? (Number(product.price) - (Number(product.discountPercentage) / 100 * Number(product.price))).toFixed(2) : Number(product.price)?.toFixed(2)}</p> 
       {product.discount &&  <p className="ml-4 bg-orange-100 rounded px-2 text-sm font-bold text-orange-500">{product.discountPercentage}%</p>}
        </div>
       ...
       <SellerDetail product={product} setChatBox={setChatBox} setBottomAdDisplay={setBottomAdDisplay} sellerId={product.sellerId}  />
        </div>
    </div>
    {expandMobileShare && <MobileShareOnSocialMedia setExpandMobileShare={setExpandMobileShare} />}
</div>
}
